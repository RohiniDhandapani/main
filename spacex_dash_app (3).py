...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        title='Total SucccessnLaunces by Site')
        return fig
    else:
        filtered_df=filtered_df[filtered_df['Launch Site'] == entered_site]
        filtered_df=filtered_df.groupby(['Launch Site','class']).size().reset_index(name='class count')
        fig = px.pie(filtered_df, values='class count',names='class', 
        title=f'Total SucccessnLaunces for Site {entered_site}')
        return fig
...
def get_pie_chart(entered_site,payload):
    low, high = payload
    filtered_df1 = spacex_df[(spacex_df['Payload Mass (kg)']>low) & (spacex_df['Payload Mass (kg)']<high)]
    if entered_site == 'ALL':
